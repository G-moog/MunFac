...
public class TokenProvider {
    ...
    public TokenDTO generateTokenDTO(Member member) {
        ...
        Claims claims = Jwts.claims().setSubject(String.valueOf(member.getMemberNo()));

        // 2. 회원의 권한들을 "auth"라는 클레임으로 토큰에 추가
        claims.put(AUTHORITIES_KEY, member.getAuthNo());
        ...
        return new TokenDTO(BEARER_TYPE, String.valueOf(member.getMemberNo()), accessToken,
                accessTokenExpiresIn.getTime());
    }

    // 2. 토큰의 등록된 클레임의 subject에서 해당 회원의 회원번호를 추출하는 메서드.
    ...
    public Authentication getAuthentication(String token) {
        ...
        UserDetails userDetails = userDetailsService.loadUserByUsername(this.getMemberNo(token));
